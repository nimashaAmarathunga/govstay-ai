            ...
            <div className="flex gap-3">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-10 pr-4 py-2 rounded-lg border border-slate-200 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 shadow-sm bg-white text-slate-800"
                  value={searchQuery}
            ...
            {filteredBungalows.map((bungalow) => (
              <div key={bungalow.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-lg transition-all group flex flex-col">
                <div className="h-48 w-full relative overflow-hidden shrink-0">
                  <img src={bungalow.image} alt={bungalow.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-md shadow-sm flex items-center gap-1">
                    ...
                    <span className="text-xs font-bold text-slate-700">{bungalow.rating}</span>
                  </div>
                </div>
                <div className="p-5 flex-1 flex flex-col">
                  <h3 className="font-bold text-lg text-slate-800 mb-1">{bungalow.name}</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]">
            ...
            <div className="md:w-1/2 h-64 md:h-auto relative">
              <img src={selectedBungalow.image} alt={selectedBungalow.name} className="w-full h-full object-cover" />
              <button
            ...
            <div className="md:w-1/2 p-8 flex flex-col overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  ...
                  <p className="text-lg font-bold text-slate-800">{selectedBungalow.noOfRooms} rooms</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <span className="material-symbols-outlined text-blue-600 mb-1">group</span>
                  <p className="text-xs text-slate-500 uppercase tracking-wider font-bold">Max Capacity</p>
                  <p className="text-lg font-bold text-slate-800">Up to {selectedBungalow.capacity} guests</p>
                </div>
              </div>
              ...
              <div className="mb-8">
                ...
                <Link href={`/browse/${selectedBungalow.slug}`} className="inline-block px-4 py-2 bg-blue-50 text-blue-600 font-semibold rounded-lg hover:bg-blue-600 hover:text-white transition-colors">
